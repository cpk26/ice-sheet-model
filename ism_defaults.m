...
if ~isfield(pd,'A'), pd.A = pd.E*3.5*10^(-25); end                 % ice rheological parameter [Pa^(-n)/s], for depth integrated flow
if ~isfield(pd,'A_b'), pd.A_b = 7*10^(-24); end                     % ice rheological parameter [Pa^(-n)/s], at base for sliding law
