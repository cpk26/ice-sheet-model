...
if ~isfield(pd,'n_rp'), pd.n_rp = 10^-5/(pd.ty); end;              % Effective Viscosity regularization parameter (m/year) -> (m/s) (Arthern et al, 2015)
if ~isfield(pd,'u_rp'), pd.u_max = 3.1710e-06; end;                % Max velocity for SIA (100m/year) -> (m/s)
