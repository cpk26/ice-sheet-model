...
[i,j] = find(gg.nbnd); %Note: finite diff gradient algorithm is not a suitable substitute.
for p=1:numel(i), 
...
AA = (E & fliplr(E)); BB = (F & flipud(F));   %Periodic BC nodes (must be on domain edge)
gg.nperbc = AA | BB;
...
%C-grid
AA = padarray(single(dd.mask),[1,1], 'symmetric'); BB = conv2(AA,[1 1; 1 1]/4, 'valid');  %need symmetric padding
